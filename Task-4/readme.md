...
hemal45/simple-node:11   ← versioned
hemal45/simple-node:latest
...
AWS Systems Manager (SSM) Run Command is used to remotely execute a deployment script on the target EC2 instance — without requiring SSH access or open inbound ports. The deployment command is base64-encoded before transmission to avoid shell escaping issues with special characters. On the EC2, it:

Updates the image tag in docker-compose.yml using sed
Pulls the new image from Docker Hub
Restarts the container with docker compose up -d --remove-orphans

Post Stage — Cleanup
Regardless of success or failure, the pipeline always cleans up the locally built Docker images from the Jenkins agent and wipes the workspace to free disk space.
...
Check Jenkins build history or Docker Hub tags to find the last known good image version, for example hemal45/simple-node:9.
...
aws ssm send-command \
  ...
  --region "ap-south-1" \
  --parameters commands=["cd /var/www/simple-app/simple-backend && sed -i 's|image: hemal45/simple-node:.*|image: hemal45/simple-node:9|g' docker-compose.yml && docker compose pull && docker compose up -d --remove-orphans"]